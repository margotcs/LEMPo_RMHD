% =========================================================================
% Example: Run IM_LEMPo_res for a m/n=9/10 mode (scan on pressure)
% =========================================================================
%
% This script demonstrates how to run the IM_LEMPo_res solver for a 
% range of pressure gradients (alpha values) for a m/n=9/10 interchange mode.
% It plots the growth rates from IM_LEMPo_res and the theoretical Mercier 
% marginal stability limit (which is an ideal result, so we obtain
% resistive growth rates even for lower alpha than predicted by the Mercier
% criterion).
%
% -------------------------------------------------------------------------
...
alpha_vec = linspace(0.005, 0.2, N_it);     % Range of alpha values (pressure gradients)
...
for i = 1:N_it
    ...
    [ev(i), result] = IM_LEMPo_res(m, n, N, ev_guess, toPlot, profiles, opts);
...
legend('LEMPo_{res}^{IM}', 'Mercier marginal stability (ideal)')
xlabel('\alpha')
