function [ev, result] = LEMPo_ideal(m, n, N, ev_guess, toPlot, profiles, opts)

% Run the full ideal model (so including infernal corrections)
%
% INPUTS:
...
resistiveSimu = 0;
% Load profiles and physical parameters (utils function)
input = getProfiles(m, n, resistiveSimu, profiles, toPlot, opts);
[q, qp, beta, betap, alpha, alphap, R0, rStar, RS, RSp, RSm, elong, triangByR] = deal( ...
...
% and sidebands
xall = gridBunching(RS, RSp, RSm, N, input.upperBound, opts);  % utils function
...
AddM = 1;  AddR = 1;   % These two appear only for the m = 1 case.

% Multiplication factors for the sideband displacement on the plots
FactorPlotSid = 1; %30;
...
deltap = @(x) q(x).^2./((x+1e-10).^3).*cumtrapz(x,x.^3./(q(x).^2.*R0) + ...
                x.^2.*alpha(x)./(q(x).^2)) ;  
...
Q = @(x) (q(x).^(-1) - 1/qs);
Qp = @(x) -qp(x)./(q(x).^2);
QPlus = @(x) (q(x).^(-1) - n/(m+1)).^2;
QPlusp = @(x) -2.*(q(x).^(-1) - n/(m+1)).*qp(x)./(q(x).^2);
QMinus = @(x) (q(x).^(-1) - n/(m-1)).^2;
QMinusp = @(x) -2.*(q(x).^(-1) - n/(m-1)).*qp(x)./(q(x).^2);
...
Xip =  V(N+1:2*N) ; Xip = Xip./maxXi;  % r. plasma disp. of upper sideband
if m~=1
   Xim =  V(2*N+1:3*N) ; Xim = Xim./maxXi; % r. plasma disp. of lower sid.
end
...
result.ev = ev % This is the growth rate NON square
 
Xip = FactorPlotSid.*Xip;
if m~=1
     Xim = FactorPlotSid.*Xim;
end
...
function defineFMidealOperators()
    ...
    if AddW1  
        if m==1
            factorW1 = @(x) dqqs(x).*qs.^(-2).*(4.*iar(x).^2.*(2 - qs.^(-2)) + iar(x).*alpha(x).*(5 - 2.*qs.^(-2)) + (3/2).*alpha(x).^2 + deltap(x).*(12.*deltap(x) - (13/2).*alpha(x) + (1/2).*x.*alphap(x) - 6.*iar(x)) ); %
            M_m_0D =  @(x) M_m_0D(x) - x.*factorW1(x);
        else
            factorW1 = @(x) dqqs(x).*qs.^(-2).*(4.*iar(x).^2.*(2 - qs.^(-2)) + iar(x).*alpha(x).*(5 - 2.*qs.^(-2)) + 2.*alpha(x).^2 + deltap(x).*(12.*deltap(x) - 7.*alpha(x) + x.*alphap(x) - 6.*iar(x)) );
            M_m_0D =  @(x) M_m_0D(x) - x.*factorW1(x);%
        end
    end
    M_m_2D = @(x) -(x.^3).*Q(x).^2;
    ...
    if AddS1
        M_sp_0D = @(x) M_sp_0D(x) + (2+m).*x.*(dqqs(x).*(iar(x).*(1+m) + alpha(x) + m.*deltap(x)))./((1+m).*qs.^2);
        M_sp_1D = @(x) M_sp_1D(x) + x.^2.*(dqqs(x).*( 2.*iar(x).*(1+m) + (2+m).*alpha(x) - (1+2.*m).*deltap(x)))./((1+m).*qs.^2);
    end
    if AddS2
    ...
    if m~=1
        ...
        M_sm_2D = @(x) 0.*x; 
        if AddS1
            M_sm_0D = @(x) M_sm_0D(x) + (2-m).*x.*(dqqs(x).*(iar(x).*(1-m) + alpha(x) - m.*deltap(x)))./((1-m).*qs.^2);
            M_sm_1D = @(x) M_sm_1D(x) + x.^2.*(dqqs(x).*( 2.*iar(x).*(1-m) + (2-m).*alpha(x) - (1-2.*m).*deltap(x)))./((1-m).*qs.^2);
        end
        if AddS2     
    ...
    % Coefficients in the upper sideband equation
    SpG_0D = @(x) -x.*m.*(m+2).*(1+2*qsp^2)./((m+1).^2);
    SpG_1D = @(x) 3.*x.^2.*(1+2.*qsp.^2)./((m+1).^2);
    SpG_2D = @(x) x.^3.*(1+2.*qsp.^2)./((m+1).^2);
    
    Sp_sp_0D = @(x) x.*m*(m+2).*QPlus(x);
    Sp_sp_1D = @(x) -3.*(x.^2).*QPlus(x) - (x.^3).*QPlusp(x);
    Sp_sp_2D = @(x) -(x.^3).*QPlus(x);  %
    ...
    if m~=1
        SmG_0D = @(x) -x.*m.*(m-2).*(1+2.*q(x).^2)./((m-1)^2);
        SmG_1D = @(x) 3.*x.^2.*(1+2.*q(x).^2)./((m-1)^2) + x.^3.*4.*qp(x).*q(x)./((m-1)^2);
        SmG_2D = @(x) x.^3.*(1+2.*q(x).^2)./((m-1)^2);
    
        Sm_sm_0D = @(x) x.*m.*(m-2).*QMinus(x);
        Sm_sm_1D = @(x) -3.*(x.^2).*QMinus(x) - (x.^3).*QMinusp(x);
        Sm_sm_2D = @(x) -(x.^3).*QMinus(x);
...
function applyBCs()
     ...
     for ii=[1,N]
         ...
         % A matrix : 
         %if m=1, need Neumann BC instead of Dirichlet on the main mode :
         M_m(ii,:) = 0; M_m(:,ii) = 0; 
         if m==1
             M_m(1,1) = 1.; M_m(1,2) = -1.;  
             M_m(2,ii) = 0; M_m(2,1) = 1.; M_m(2,3) = -1.; M_m(N,N) = 1;
             MG(2,ii) = 0.; M_sp(2,:) =0.;
         else
             M_m(ii,ii) = 1.;
         end
         ...
         if m~=1
             ...
             if m == 2   
                % Apply Neumann BC instead of dirichlet on the lower sideband
...
% Here we build A and B to solve A v = gamma/omegaA B v
    ...
    if NeumannBC % in this case, no sideband inertia
        SpG = Z;   
    end
    
    if m==1 % in this case, no lower sideband to handle
        B = [ MG,   Z;
                Z,  SpG];
...
function makePlots()
    ...
    if m~=1
        plot(xall, Xi, '.-',xall, Xip, '.-', xall, Xim, '.-') ;
        title(['m = ',num2str(m),' ,n= ',num2str(n),' ,\alpha(', num2str(rStar),') = ',num2str(alpha(rStar)),', \gamma / \omega_A =  ',num2str(ev)],'interpreter','tex', 'FontWeight', 'normal');
        for i=1:max(size(RS))
            xline(double(RS(i)), '--b', 'LineWidth', 2);
        end
        if ~isempty(RSp)
            xline(RSp, '--r', 'LineWidth', 2);
        end
        legend('\xi^{(m)}', [num2str(FactorPlotSid),' \xi^{(m+1)}'], [num2str(FactorPlotSid),' \xi^{(m-1)}'])
        ylabel('\xi [a.u]');
        xlabel('r / a');
        
    else
        plot(xall, Xi, '.-',xall, Xip, '.-') ;
        ...
        legend('\xi^{(m)}', [num2str(FactorPlotSid),' \xi^{(m+1)}'])
        ylabel('\xi [a.u]');
        xlabel('r / a');
    end
